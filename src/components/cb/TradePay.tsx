...
export function TradeTab() {
  ...
  const [toId, setToId] = useState(state.tokens[1]?.id ?? "eth");
    ...
    <div className="flex h-full flex-col px-4 pt-4">
      ...
      <div className="mt-3 flex rounded-full bg-[#1e2026] p-1 text-[13px] font-semibold">
        {(["buy", "sell", "convert"] as const).map((m) => (
          <button
            ...
            type="button"
            onClick={() => setMode(m)}
            className={`flex-1 rounded-full py-2 capitalize ${mode === m ? "bg-[#0052FF]" : ""}`}
          >
            {m}
          </button>
        ))}
      ...
      {mode === "buy" && (
        <>
          ...
          <input className="field text-[22px] font-semibold" value={usd} onChange={(e) => setUsd(e.target.value)} />
      ...
      {mode === "sell" && (
        <>
          ...
          <input className="field text-[22px] font-semibold" value={qty} onChange={(e) => setQty(e.target.value)} />
          {token && (
            <p className="mt-2 text-[13px] text-white/45">
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 px-4">
        ...
        <button
          ...
          className="h-12 rounded-full bg-[#1e2026] text-[14px] font-semibold"
        >
          Receive
        </button>
      </div>
      ...
      <ul className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        {state.activity.map((a) => (
          <li key={a.id} className="border-b border-white/5 py-3">
            ...
            <p className="mt-1 text-[12px] text-white/45">{a.status} · {a.at}</p>
